...
export function usePlayroomState() {
  ...
  useEffect(() => {
    const interval = window.setInterval(() => { const snapshot = captureSnapshot(state, "interval"); if (snapshot) setLastSnapshotAt(snapshot.capturedAt); }, state.systemConfig.autoCaptureIntervalSec * 1000);
    const online = () => { setIsOnline(true); flushSyncQueue(); }; const offline = () => setIsOnline(false);
  ...
  const captureNow = useCallback(() => { const snapshot = captureSnapshot(state, "manual"); if (snapshot) setLastSnapshotAt(snapshot.capturedAt); return snapshot; }, [state]);
  ...
  const finishSession = useCallback((stationId) => updateState((current) => {
    const station = current.stations.find((item) => item.id === stationId); if (!station?.activeSession) return current;
    const session = station.activeSession; const hours = Math.max(1, (Date.now() - session.startTime) / 3600000); const rate = station.pricing[session.controllerCount > 2 ? "multiRatePerHour" : "singleRatePerHour"]; const sessionTotal = Math.round(hours * rate) + session.ordersTab.reduce((sum, item) => sum + item.price * item.qty, 0);
