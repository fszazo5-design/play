...
import { BarChart3, CalendarClock, Check, CreditCard, Download, Grid3X3, LayoutDashboard, MonitorPlay, Package, Settings2, Trophy, Users, X } from "lucide-react";
...
const navigation = [
    ...
    { id: "modules", label: "شبكة الوحدات", icon: Grid3X3 },
...
export default function App() {
    ...
    return (<div className="app-shell" dir="rtl">
      ...
      <aside className="sidebar">
        ...
        <button type="button" className="branch-pill" onClick={() => setActiveView("settings")} aria-label="فتح إعدادات الفرع"><span className="pulse-dot"/><div><strong>{playroom.state.systemConfig.branchId}</strong><small>الفرع الرئيسي · متصل</small></div><Settings2 size={15}/></button>
        <nav className="side-nav" aria-label="التنقل الرئيسي"><p className="nav-label">إدارة الصالة</p>{navigation.map((item) => { const Icon = item.icon; const selected = activeView === item.id || (activeView === "stations" && item.id === "overview"); return <button type="button" aria-label={item.label} title={item.label} className={`nav-item ${selected ? "selected" : ""}`} onClick={() => setActiveView(item.id)} key={item.id}><Icon size={18} strokeWidth={selected ? 2.4 : 1.8}/><span>{item.label}</span>{item.id === "stations" && <b>{playroom.metrics.activeStations}</b>}{item.id === "inventory" && playroom.metrics.lowStockItems > 0 && <b className="nav-alert">{playroom.metrics.lowStockItems}</b>}</button>; })}</nav>
